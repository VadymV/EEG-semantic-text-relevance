...
class DatasetSentences(Dataset):
    ...
    def read_data(self) -> dict:
        ...
        for idx in range(len(self.documents)):
            ...
            eeg, text = load_data(
                folder_path=sample_path, file_name=self.documents[idx]
            )

            text.loc[:, "user"] = re.search("(TRPB)[0-9]{3}", sample_path).group()
            text = text.reset_index(drop=True)
            text["sentence_relevance"] = 0
            text.loc[text["topic"] == text["selected_topic"], "sentence_relevance"] = 1
            ...
            for sen in number_sentences:
                ...
                for word_eeg in sentence_eeg:
                    ...
                    eeg_mean_groups = [torch.mean(i, dim=-1) for i in eeg_groups]
                    eeg_ = torch.stack(eeg_mean_groups, dim=-1)
                    eeg_ = eeg_.reshape(eeg_.shape[0], -1)
    ...
    def get_participants(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        participants = np.array(
            [p.loc[:, "user"].tolist()[0] for p in self.data["text"]]
        )
    ...
    def get_indices_for_reading_tasks(self, participant_indices: list) -> OrderedDict:
        ...
        blocks = (
            d[["topic", "selected_topic", "sentence_relevance", "event"]]
            .groupby(by=["event"])
            .min()
            .drop_duplicates()
            .sort_index()
        ...
        unique_blocks.sort()
        for block in blocks:
            block_test_indices[block] = [
                participant_indices[i] for i, x in enumerate(blocks) if x == block
            ]
...
class CollatorSentence(object):
    ...
    def __call__(self, batch):
        ...
        sentence_labels = [d["sentence_relevance"].tolist().pop() for d in text_data]
...
class CollatorEEGNetSentence(object):
    ...
    def __call__(self, batch):
        ...
        document_labels = [d["sentence_relevance"].tolist().pop() for d in text_data]
...
class CollatorLSTMSentence(object):
    ...
    def __call__(self, batch):
        ...
        packed_sequence = torch.nn.utils.rnn.pack_sequence(
            eeg_batch, enforce_sorted=False
        )
        document_labels = [d["sentence_relevance"].tolist().pop() for d in text_data]
...
class CollatorTransformerSentence(object):
    ...
    Args:
        max_length: The sequence length to pad/truncate to.
    """

    def __init__(self, max_length: int = MAX_SENTENCE_LENGTH):
    ...
    def __call__(self, batch):
        ...
        truncated_eeg = [d[: self.max_length] for d in eeg_batch]
        padded_eeg = [
            torch.nn.functional.pad(d, pad=(0, 0, 0, self.max_length - d.shape[0]))
            for d in truncated_eeg
        ]
        padded_eeg = torch.stack(padded_eeg)
        sentence_lengths = [d.shape[0] for d in truncated_eeg]
